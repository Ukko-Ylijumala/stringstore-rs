...
`TextElement`, `StructuredLine`, `Character`, `CompactStr`, `Hex`, `HexFormat`, and the `Integer` trait compile but are not wired up to any public API and have no tests. They are an in-progress sketch of a structured-text parsing feature. The crate uses `#![allow(dead_code)]` to keep this compiling. Treat the shapes of these types as unstable — feel free to rework them when implementing the feature for real.
